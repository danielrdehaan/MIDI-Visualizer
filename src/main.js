...
var require_midi_parser = __commonJS({
  "node_modules/midi-file/lib/midi-parser.js"(exports, module2) {
...
var require_midi_writer = __commonJS({
  "node_modules/midi-file/lib/midi-writer.js"(exports, module2) {
  ...
  async onload() {
    this.registerMarkdownCodeBlockProcessor("midiviz", async (source, el, ctx) => {
      ...
      if (audioFilename) {
        ...
        if (foundAudio instanceof import_obsidian.TFile) audioFile = foundAudio;
      }
      if (!midiFile || !(midiFile instanceof import_obsidian.TFile)) {
    ...
    if (sortedTempos.length > 0) {
      for (const t of sortedTempos) {
        this.customTempoMap.push({
          ...
          bpm: t.bpm,
          time: t.time
          // Use the library's pre-calculated time
        });
      }
  ...
  renderInteractivePianoRoll(midi, container, options) {
    ...
    const getCurrentTime = () => {
      if (audioElement) {
        ...
        return now() - synthStartTime + synthPauseTime;
      } else if (useSynth) {
        return synthPauseTime;
      }
      return 0;
    };
    const scheduleNotes = (startTimeSeconds) => {
      if (!synth) return;
        ...
        track.notes.forEach((note) => {
          ...
          if (noteStartTime >= 0) {
            const eventId = Transport.schedule((time) => {
              synth == null ? void 0 : synth.triggerAttackRelease(
                Frequency(note.midi, "midi").toFrequency(),
                note.duration,
                time,
                note.velocity
              );
    ...
    if (options.midiFile) {
      const revealBtn = controlsBar.createEl("button", { text: "\u{1F3B9} Reveal", cls: "midi-reveal-btn" });
      ...
      revealBtn.onclick = () => {
        if (options.midiFile) {
          this.app.showInFolder(options.midiFile.path);
        }
      };
    }
    ...
    if (synth) {
      this.register(() => {
        stopSynth();
        synth == null ? void 0 : synth.dispose();
      });
    }
    ...
    const handlePointerDown = (x, y, isTouch = false) => {
      dragStartX = x;
      dragStartY = y;
      lastPointerX = x;
      lastPointerY = y;
    ...
    const handlePointerMove = (x, y) => {
      ...
      if (isDraggingPlayhead) {
        ...
        if (Math.abs(movementY) > 0) {
          ...
          if (movementY > 0) zoomX *= zoomFactor;
          else zoomX /= zoomFactor;
          applyConstraints();
    ...
    canvas.addEventListener("touchstart", (e) => {
      if (e.touches.length === 2) {
        ...
        handlePointerDown(pos.x, pos.y, true);
      }
    }, { passive: false });
